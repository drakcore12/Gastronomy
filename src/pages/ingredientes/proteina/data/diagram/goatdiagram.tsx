...
import { HOTSPOTS_GOAT, type Hotspot } from '../hotspots/hotspots';
...
export default function GoatDiagram() {
  /** =========================
   *  🎛️ CONFIGURACIÓN EDITABLE
   *  ========================= */

  // Tema de colores
  const theme = DIAGRAM_THEMES.goat;
  ...
  const SVG_ARIA_LABEL = 'Diagrama de cortes de cabra';
  ...
  const sel = useMemo<Hotspot | null>(
    () => HOTSPOTS_GOAT.find((h) => h.id === active) ?? null,
    [active]
  );
