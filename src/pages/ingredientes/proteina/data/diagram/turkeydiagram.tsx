...
import { HOTSPOTS_TURKEY, type Hotspot } from '../hotspots/hotspots';
import { DIAGRAM_THEMES } from './theme-colors';

export default function TurkeyDiagram() {
  /** =========================
   *  🎛️ CONFIGURACIÓN EDITABLE
   *  ========================= */

  // Tema de colores
  const theme = DIAGRAM_THEMES.turkey;
  ...
  const SVG_ARIA_LABEL = 'Diagrama de cortes de pavo';
  ...
  const sel = useMemo<Hotspot | null>(
    () => HOTSPOTS_TURKEY.find((h) => h.id === active) ?? null,
    [active]
  );
        ...
        {HOTSPOTS_TURKEY.map((h) => {
          ...
          return (
            ...
            >
              ...
              >
                {HOTSPOTS_TURKEY.findIndex(spot => spot.id === h.id) + 1}
              </text>
